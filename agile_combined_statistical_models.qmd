...
### adding optional code to combine neutrophil populations to understand overall changes

```{r}
blood_df <- blood_df %>% mutate(label = if_else(label %in% c("Neutrophil_mature_blood", "Neutrophil_immature_blood", "Neutrophil_aged_blood"), "Neutrophil_blood", label))
blood_df2 <- blood_df %>%
  group_by(donor, treatment_clean, time_clean, label, Age) %>%
  summarise(
    count = sum(count),
    prop = sum(prop),
    .groups = "drop"
...
blister_df <- blister_df %>% mutate(label = if_else(label %in% c("Neutrophil_mature_blister", "Neutrophil_aged_blister"), "Neutrophil_blister", label))
blister_df2 <- blister_df %>%
  group_by(donor, treatment_clean, time_clean, label, Age) %>%
  summarise(
    count = sum(count),
    prop = sum(prop),
...
```{r}
emm_myeloid <- filter(emm_all, label %in% c("Classical_mono_blood", "DC_blister", "Treg_blister"))
...
ggsave(plot = longitudinal_myeloid, filename = "myeloid_props.png", dpi=300, height = 3, width=10)
```
...
neut_names <- c("Neutrophil_mature_blood", "Neutrophil_mature_blister", "Neutrophil_aged_blood", "Neutrophil_immature_blood", "Neutrophil_aged_blister")
neut_markers <- c("CD16", "CD14", "CCR6", "CD47","CD66b", "HLA-DR", "CD182", "CD184", "CD15", "CD11b", "CD62L")

labels <- labels[labels %in% c("Treg_blister")]
markers <- unique(mean_exp$marker)
markers <- markers[markers %in% c("FOXP3", "CTLA.4", "TIGIT", "PD.1", "CCR6", "CD103", "CD127", "CD25", "CD69", "CD28", "KI.67")]
...
p <- ggplot(heatmap_df, aes(x = time_clean, y = marker, fill = estimate_capped)) +
  ...
  labs(title = "Difference in blister Treg \n phenotype in Corona - Guinness",
       x = "Timepoint", y = NULL)+
        facet_wrap(~label, scales = "free_y", ncol=1)

p
ggsave(plot=p,filename="treg_blister_marker_change_heatmap.png", dpi=300, height=5,width=5)
...
#emm_markers <- filter(emm_results, marker %in% c("FOXP3", "CTLA4", "TIGIT", "PD.1", "HLA.DR", "CD80", "CD86"))

p <- ggplot(emm_results, aes(x = time_clean, y = emmean, color = treatment_clean, group = treatment_clean)) +
  geom_point(size = 3) +
  geom_line(size = 1.2) +
  geom_errorbar(aes(ymin = emmean - SE, ymax = emmean + SE), width = 0.2) +
...
p
ggsave(plot=p,filename="treg_blister_longitudinal.png", dpi=300,height=10,width=12)
```
