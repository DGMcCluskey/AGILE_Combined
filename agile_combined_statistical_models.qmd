...
summary_df <-  pivot_longer(summary_df, names_to="marker", values_to="mean_expression", cols=7:37)
summary_df$time <- factor(summary_df$time, levels = c("-72h", "0h", "4h", "6h", "24h", "48h", "72h"), labels = c("-72h", "0h", "6h", "6h", "24h", "48h", "72h"))
```
...
```{r}
neuts <- filter(summary_df, label %in% c("Neutrophil_mature_blood", "Neutrophil_mature_blister", "Neutrophil_aged_blood", "Neutrophil_young_blood", "Neutrophil_aged_blister"))
neuts <- filter(neuts, marker %in% c("CD16", "CD62L", "CD182", "CD184", "CD15"))
ggplot(neuts, aes(x=time, y = marker, fill = mean_expression))+geom_tile()+facet_wrap(~label)
```

```{r}
markers <- unique(neuts$marker)
celltype <- c("Neutrophil_aged_blister")
tissue_type <- "blister"
model_input <- filter(neuts, label %in% celltype, tissue == tissue_type)
model_input$time <- droplevels(model_input$time)
analyse_marker <- function(marker_name) {
  dat_marker <- model_input %>% filter(marker == marker_name)
  print(marker_name)
  print(table(dat_marker$treatment, dat_marker$time))
  model <- tryCatch({
    lmer(
      mean_expression ~ treatment * time + (1 | donor),
      data = dat_marker
    )
  }, error = function(e) {
  ...
  emm <- emmeans(model, ~ treatment | time, type = "response")
...
```{r}
treg_markers <- c("FOXP3", "CD25", "CTLA4", "CD28", "CD127", "CD39", "CD62L", "CCR6", "CD69", "PD1", "TIGIT", "KI67", "HLA.DR", "CD47")
myeloid_markers <- c("CD14", "CD16", "CD11b", "CD1c", "CD15", "CD39", "CD47", "CD62L", "CD69", "CD86", "HLA.DR", "KI67")
CD8_markers <- c("CD25", "CD28", "CD47", "CD62L", "CCR6", "CD69", "PD1", "KI67", "CD103")
neut_markers <- c("CD16", "GLUT1", "CXCR2", "TIGIT", "CD11b", "CD1c", "CD15", "CD66b", "CD62L", "CD28", "CD86", "KI67")
NK_markers <- c("CD25", "CD16", "CD56", "CD28", "CD47", "CD62L", "CD69", "KI67", "CD103")
...
plot1 <- ggplot(emm_all %>% filter(marker %in% marker_list), aes(x = time_new, y = emmean, colour = treatment, group = treatment)) +
