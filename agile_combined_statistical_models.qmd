...
heatmap_df <- interaction_results %>%
  ...
  mutate(FDR = case_when(
    p_adj_fdr < 0.001 ~ "< 0.001",
    p_adj_fdr < 0.01  ~ "< 0.01",
    p_adj_fdr < 0.05  ~ "< 0.05",
    TRUE ~ ""
  ))

ggplot(heatmap_df, aes(x = time_clean, y = label, fill = estimate, shape = FDR)) +
  geom_point(colour="black", size = 8)+
  #geom_text(aes(label = sig), color = "black", size = 10, vjust=2) +
...
```{r}
summary_df <- read.csv("marker_avg_expression_df.csv")
...
```{r}
ggplot(summary_df, aes(x=time, y = marker, fill = mean_expression))+geom_tile()+facet_wrap(~label)
```