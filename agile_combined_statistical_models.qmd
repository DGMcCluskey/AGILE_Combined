...
ggplot(emm_all, aes(x = time_clean, y = emmean, color = treatment_clean, group = interaction(treatment_clean, tissue), linetype = tissue, shape = tissue)) +
  geom_point(size = 3) +
  geom_line(size = 1.2) +
  geom_errorbar(aes(ymin = emmean - SE, ymax = emmean + SE), width = 0.2) +
  ...
  theme(axis.text = element_text(colour="black", size = 14),
  strip.text=element_text(colour="black", size=16, face="bold"))+
    ylab("Estimated % of blood/blister cells")
```