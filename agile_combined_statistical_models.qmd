...
labels <- labels[labels %in% c("Classical_mono_blood")]
markers <- unique(mean_exp$marker)
markers <- markers[markers %in% c("CD14", "CD16", "CD80", "CD86", "CTLA4", "HLA-DR", "CD1c", "KI.67")]
...
p <- ggplot(heatmap_df, aes(x = time_clean, y = marker, fill = estimate_capped)) +
  ...
  labs(title = "Difference in classical monocyte \n phenotype in Corona - Guinness",
       x = "Timepoint", y = NULL)+
        facet_wrap(~label, scales = "free_y", ncol=1)

p
ggsave(plot=p,filename="monocyte_marker_change_heatmap.png", dpi=300, height=5,width=5)
...
p
ggsave(plot=p,filename="monocyte_marker_longitudinal.png", dpi=300,height=10,width=12)
```
