...
# Compute proportions within each donor × time × treatment
blister_counts["prop"] = blister_counts.groupby(["donor", "time", "treatment_clean"])["count"].transform(lambda x: x / x.sum())

```

```{python}
time_order = ["-72h", "0h", "4h", "6h", "24h", "48h", "72h"]
blood_counts['time'] = pd.Categorical(
    blood_counts['time'],
