...
cell_annotation_dict = {
    ...
    "18": "Debris/doublets",
    "19": "NK_blood",
    "20": "Neutrophil_mature_blister",
    "21": "CD4+T_blood",
    "22": "Neutrophil_aged_blood",
    "23": "Classical_mono_blood",
    "24": "Invariant_T_blister",
    "25": "Neutrophil_aged_blister",
    "26": "CD4+T_blood",
    "27": "Neutrophil_immature_blood",
...
cluster_palette= ["indianred", "green", "limegreen", "darkorchid", "purple", "darkred", "red", "chocolate", "grey", "hotpink", "goldenrod", "darkgoldenrod", "darkcyan", "darkblue", "cyan", "deepskyblue", "blue", "greenyellow"]
```
...
## also remove neuts as their cd16 expression is so high i can't find monocyte subsets
```{python}
remove = ["Debris/doublets", "Invariant_T_blister", "Neutrophil_immature_blood", "Neutrophil_mature_blood", "Neutrophil_aged_blood", "Neutrophil_mature_blister", "Neutrophil_aged_blister"]
adata_trim = adata[~adata.obs["label"].isin(remove)].copy()
sc.tl.dendrogram(adata_trim, groupby="label")
plt.figure()
sc.pl.matrixplot(
    adata_trim,
    var_names=["CD3", "CD4", "CD8", "FOXP3", "CD25", "CD127", "TIGIT", 
    #"CD56", "CD14", "CD16", "CD1c", "HLA-DR", "CD66b", "CD15", "CD62L"
    ],
    groupby="label",
    layer="imputed",
    dendrogram=True,
...
```{python}
plt.figure()
sc.pl.umap(adata_trim, color = ["FOXP3", "CTLA-4", "TIGIT", "PD-1", "CCR6", "CD103", "CD127", "CD25", "CD69", "CD28", "KI-67"], cmap = "magma_r", layer = "imputed", size = 10, legend_fontsize=12)
plt.savefig("umap_trimmed_treg_markers.png", dpi=300)
```
...
```{python}
sc.pl.stacked_violin(adata_sub, var_names=["CD16", "CD62L", "CD15", "CD182", "CD184"], groupby="label", layer = "imputed", swap_axes=True, standard_scale="var", cmap="Reds", var_group_rotation=45, categories_order=["Neutrophil_immature_blood", "Neutrophil_mature_blood", "Neutrophil_aged_blood", "Neutrophil_mature_blister", "Neutrophil_aged_blister"])
plt.savefig("neitrophil_phenotype_violins.png", dpi=300, bbox_inches="tight")
```

```{python}
# Count and convert to percentages
cluster_counts = adata.obs["label"].value_counts(normalize=True) * 100

cluster_counts = cluster_counts.reset_index()
cluster_counts.columns = ["label", "percent"]

plt.figure()
sns.barplot(data=cluster_counts, x="label", y="percent", palette=cluster_palette)
plt.xticks(rotation=90)
plt.ylabel("Percent of all cells")
plt.title("Cluster proportions")
plt.tight_layout()
plt.savefig("proportion_total.png", dpi=300, bbox_inches="tight")
plt.show()
