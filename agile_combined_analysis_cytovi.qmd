...
blood_counts = (
    df_blood.groupby(["donor", "time", "treatment_clean", "label"])
      .size()
      .reset_index(name="count")
)

# Compute proportions within each donor × time × treatment
blood_counts["prop"] = blood_counts.groupby(["donor", "time", "treatment_clean"])["count"].transform(lambda x: x / x.sum())

# Count cells per donor × time × treatment × label
blister_counts = (
    df_blister.groupby(["donor", "time", "treatment_clean", "label"])
      .size()
      .reset_index(name="count")
)

# Compute proportions within each donor × time × treatment
bister_counts["prop"] = blister_counts.groupby(["donor", "time", "treatment_clean"])["count"].transform(lambda x: x / x.sum())
...
```{python}
p = (
    ggplot(blood_counts, aes(x="time", y = "prop", colour="treatment_clean", group = d))+geom_point()+
    facet_wrap("label", scales="free_y")
)
p.show()
p.save("proportions_time.png", dpi=300, width=10,height=8)
```