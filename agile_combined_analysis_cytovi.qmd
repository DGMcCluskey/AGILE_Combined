...
```{python}
marker = "CD56"
layer = "imputed"   # or None for raw X

# Pull data for the marker
values = (
    adata.layers[layer][:, adata.var_names == marker].flatten()
    if layer is not None else
    adata[:, marker].X.flatten()
)
...
g = sns.FacetGrid(
    df, 
    col="label",         # facet variable — change to label/treatment/etc.
    col_wrap=3,
    sharex=True,
    sharey=False
)
g.map_dataframe(sns.histplot, x=marker, bins=50, kde=False)
